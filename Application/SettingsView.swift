    ...
    var body: some View {
        VStack {
            VStack(alignment: .leading, spacing: 20) {
                PrimarySettingsView(settings: settings)
                Divider()
                SecondarySettingsView(settings: settings)
            }
            Spacer(minLength: 35)
            HStack {
                Button("Save Changes") {
...
struct PrimarySettingsView: View {
    @ObservedObject var settings: Settings
    private var factory: LiteratorFactory
    
    init(settings: Settings) {
        self.settings = settings
        self.factory = try! LiteratorFactory(config: LipikaConfig())
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 17) {
            ...
            VStack(alignment: .center, spacing: 4) {
                HStack {
                    Text("If you type")
                    TextField("Character", text: $settings.stopCharacter)
            ...
            HStack {
                ...
                TextField("Character", text: $settings.escapeCharacter)
                    .textFieldStyle(RoundedBorderTextFieldStyle())
...
struct SecondarySettingsView: View {
    @ObservedObject var settings: Settings
